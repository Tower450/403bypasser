...
print(
    Fore.BLUE + Style.BRIGHT + custom_fig.renderText("-------------") + Style.RESET_ALL
)
print(Fore.BLUE + Style.BRIGHT + custom_fig.renderText("403bypasser") + Style.RESET_ALL)
print(
    Fore.GREEN
    + Style.BRIGHT
    + "____________________ Yunus Emre SERT ____________________\n"
)
print(
    Fore.LIGHTMAGENTA_EX
    + Style.BRIGHT
    + "-----> Twitter   : https://twitter.com/yunem_se\n"
)
print(Fore.MAGENTA + Style.BRIGHT + "-----> GitHub    : https://github.com/yunemse48\n")
print(
    Fore.MAGENTA
    + Style.BRIGHT
    + "-----> LinkedIn  : https://www.linkedin.com/in/yunus-emre-sert-9102a9135/\n"
)
print(
    Fore.BLUE + Style.BRIGHT + custom_fig.renderText("-------------") + Style.RESET_ALL
)
...
parser.add_argument(
    "-u", "--url", type=str, help="single URL to scan, ex: http://example.com"
)
parser.add_argument(
    "-U", "--urllist", type=str, help="path to list of URLs, ex: urllist.txt"
)
parser.add_argument(
    "-d",
    "--dir",
...
parser.add_argument(
    "-D", "--dirlist", type=str, help="path to list of directories, ex: dirlist.txt"
)
    ...
    def checkURL(self):
        if self.url:
            if not validators.url(self.url):
                print(
                    "You must specify a valid URL for -u (--url) argument! Exitting...\n"
                )
                sys.exit
    ...
    def checkDir(self):
        if self.dir:
            ...
            if not os.path.exists(self.dirlist):
                print(
                    "The specified path to directory list does not exist! Exitting...\n"
                )
                sys.exit()
...
class Query:
    ...
    def manipulateRequest(self):
        print(
            (" Target URL: " + self.url + "\tTarget Path: " + self.dir + " ").center(
                121, "="
            )
        )
        results = []
        ...
        if p.status_code == 200:
            safe_path = self.dir.replace("/", "_") if self.dir != "/" else "_rootPath"
            filename = f"{self.domain}_{safe_path}.txt"
            with open(filename, "wb") as f:  # use wb for binary safety
                f.write(p.content)
    ...
    def manipulatePath(self):
        ...
        for path in self.dirObject.newPaths:
            ...
            if r.status_code == 200:
                ...
                filename = f"{self.domain}_{safe_path}.txt"
                with open(filename, "wb") as f:  # now filename exists
                    f.write(r.content)
    ...
    def manipulateHeaders(self):
        ...
        for header in self.dirObject.newHeaders:
            ...
            if r.status_code == 200:
                safe_header = (
                    "_".join(f"{k}-{v}" for k, v in header.items())
                    .replace("/", "_")
                    .replace(" ", "_")
                )
                filename = f"{self.domain}_{self.dir.strip('/').replace('/', '_')}_{safe_header}.txt"
        ...
        for header in self.dirObject.rewriteHeaders:
            r = requests.get(
                self.url, headers=header, allow_redirects=False, verify=False
            )
...
class Program:
    ...
    def initialise(self):
        paths = {}  # store PathRepository objects
        queries = {}  # store Query objects

        for u in self.urllist:
            for d in self.dirlist:
                # 1. Create a safe key from the directory
                dir_key = d.lstrip("/").replace("/", "_") if d != "/" else "_rootPath"
                # 2. Store the PathRepository in our dict
                paths[dir_key] = PathRepository(d)
                ...
                query_key = f"{domain_name}_{dir_key}"
                queries[query_key] = Query(u, d, paths[dir_key])

                # 5. Run the logic
                queries[query_key].manipulateRequest()
